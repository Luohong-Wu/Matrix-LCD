...
void LCD::initialize() {
  st7565_init();
  st7565Command(CMD_DISPLAY_ON);
  st7565Command(CMD_SET_ALLPTS_NORMAL);
...
void LCD::st7565_init(void) {
  ...
  // LCD bias select
  st7565Command(CMD_SET_BIAS_7);
  // ADC select
  st7565Command(CMD_SET_ADC_NORMAL);
  // SHL select
  st7565Command(CMD_SET_COM_NORMAL);
  // Initial display line
  st7565Command(CMD_SET_DISP_START_LINE);

  // turn on voltage converter (VC=1, VR=0, VF=0)
  st7565Command(CMD_SET_POWER_CONTROL | 0x4);
  // wait for 50% rising
  _delay_ms(50);

  // turn on voltage regulator (VC=1, VR=1, VF=0)
  st7565Command(CMD_SET_POWER_CONTROL | 0x6);
  // wait >=50ms
  _delay_ms(50);

  // turn on voltage follower (VC=1, VR=1, VF=1)
  st7565Command(CMD_SET_POWER_CONTROL | 0x7);
  // wait
  _delay_ms(10);

  // set lcd operating voltage (regulator resistor, ref voltage resistor)
  st7565Command(CMD_SET_RESISTOR_RATIO | 0x6);
...
void LCD::st7565Command(uint8_t c) {
  digitalWrite(a0, LOW);

  spiwrite(c);
}

void LCD::st7565Data(uint8_t c) {
  digitalWrite(a0, HIGH);

  spiwrite(c);
}
void LCD::st7565SetBrightness(uint8_t val) {
    st7565Command(CMD_SET_VOLUME_FIRST);
    st7565Command(CMD_SET_VOLUME_SECOND | (val & 0x3f));
}
  ...
  */
  ...
  for(p = 0; p < 8; p++) {
    ...
    */
    ...
    st7565Command(CMD_SET_PAGE | pagemap[p]);
    ...
    st7565Command(CMD_SET_COLUMN_LOWER | ((col+ST7565_STARTBYTES) & 0xf));
    st7565Command(CMD_SET_COLUMN_UPPER | (((col+ST7565_STARTBYTES) >> 4) & 0x0F));
    st7565Command(CMD_RMW);
    
    for(; col <= maxcol; col++) {
      //uart_putw_dec(col);
      //uart_putchar(' ');
      st7565Data(st7565_buffer[(128*p)+col]);
    }
  }
...
void LCD::clearDisplay(void) {
  ...
  for(p = 0; p < 8; p++) {
    ...
    */
    ...
    st7565Command(CMD_SET_PAGE | p);
    for(c = 0; c < 129; c++) {
      //uart_putw_dec(c);
      //uart_putchar(' ');
      st7565Command(CMD_SET_COLUMN_LOWER | (c & 0xf));
      st7565Command(CMD_SET_COLUMN_UPPER | ((c >> 4) & 0xf));
      st7565Data(0x0);
    }     
  }
}
